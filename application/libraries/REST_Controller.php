...
abstract class REST_Controller extends CI_Controller {
    ...
    /**
     * @desc all methods which allows without check access token
     * @var array
     */
    ...
    {
        header("Access-Control-Allow-Headers: Origin, X-Requested-With, Content-Type, Accept, Access-Control-Request-Method,Api-Version,Device-Type,Device-Id,App-Version,Language");
        parent::__construct();
    ...
    private function _matchUserToken($method) {

        //$this->config->set_item('language','arabic');
        //$this->session->set_userdata('site_lang','english');
            //echo "here";
        //echo "--".CI::$APP->session->sess_destroy();

        $data = $this->{$this->request->method}();
        
        $current_method = $method . '_'.$this->request->method;
        
        if(!in_array($current_method, $this->allow)) {
            if(isset($data['user_token']))
            {
                $token = $data['user_token'];
                $this->user = isValidToken($token);

                if(!$this->user) {
                    $this->response(array(
                        "status" => false,
                        "message" => 'User access token not match',
                        "logout" =>true,
                        "data" => $data
                    ));
                    exit;
            ...
            else
            {
                $this->response(array(
                    "status" => false,
                    "message" => lang('User access token not passed'),
                    "logout" =>true,
                    "data" => $data
                ));
                exit;
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        $this->_matchUserToken($object_called);
    ...
    protected function _detect_api_key()
    {
        ...
        {
            $row = $this->rest->db->where($this->config->item('rest_key_column'), $key)->get($this->config->item('rest_keys_table'))->row();
    ...
    protected function _detect_lang()
    {
       ...
       $lang = $this->input->server('HTTP_ACCEPT_LANGUAGE');
        //$lang = $this->input->get_request_header('language', TRUE);
    ...
    protected function _log_request($authorized = FALSE)
    {
        ...
        $api_version = isset($this->_args['Api-Version']) ? $this->_args['Api-Version'] : $this->input->get_request_header('Api-Version');
        $device_type = isset($this->_args['Device-Type']) ? $this->_args['Device-Type'] : $this->input->get_request_header('Device-Type');
        $device_id = isset($this->_args['Device-Id']) ? $this->_args['Device-Id'] : $this->input->get_request_header('Device-Id');
    ...
    {
        //printArray($this->input->input_stream(),1);
    ...
    function sendPushToOtherUser($userId,$otherUserId,$messageId){
		$this->load->model('Common','CommonModel');
		
		$iosTokenArray 		= 	array();
		$androidTokenArray 	= 	array();
		$otherUserDetails 	=	$this->CommonModel->getUserDetail($otherUserId);
		$userDetails 		=	$this->CommonModel->getUserDetail($userId);
		...
		if($userDetails){
			$fromUserName =  $userDetails->firstname.' '.$userDetails->lastname;	
		}
	

		if($otherUserDetails){
			$androidToken 	= 	$otherUserDetails->android_token;
			$iosToken 		= 	$otherUserDetails->ios_token;

			if($androidToken != ""){
				array_push($androidTokenArray, $androidToken);
			}

			if($iosToken != ""){
				array_push($iosTokenArray, $iosToken);				
			}
		}



		if(count($iosTokenArray) > 0 || count($androidTokenArray)  > 0){
			$this->db->where("id",$messageId);
			$data = $this->db->get("user_has_notifications")->row();
			$payloadArray = array();
			...
			//}
			$this->load->model('Usernotification');
			$totalUnreadNotificationCount	=	$this->Usernotification->getUserNotificationsCount($otherUserId, 0);
			$payloadArray["title"] 		= $data->title;//$fromUserName." ".GLOBAL_SEND_NEW_MESSAGE;
			$payloadArray["badge"] 		= $totalUnreadNotificationCount;  // these are badges for android and ios 
			$payloadArray["chat_notification"] 	= "true";
			...
			if(count($androidTokenArray)  > 0){
			    $payloadArray["message"] 	= $body;
			    
		       	$this->CommonModel->send_android_push_message($androidTokenArray, $payloadArray,$isMerchant);		
			}

			if(count($iosTokenArray)  > 0){
			    $payloadArray["sound"] 		= "default";
			    $payloadArray["body"] 		= $body;
				$this->CommonModel->send_push_ios($iosTokenArray, $payloadArray, $isMerchant);		
			}


			// note title and body are compalsary in ios payload and title and message is in android payload 
		}
	}
