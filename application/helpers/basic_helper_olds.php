...
if (!function_exists('getUnreadNotificationCountByUserId')) {
   function getUnreadNotificationCountByUserId($userid){
     ...
     $myNotification = $obj->Common->total_count("tbl_notification", "user_id",array("user_id"=>"0","created >= "=>$currentDate));     
     $allNotification = $obj->Common->total_count("tbl_notification", "user_id",array("user_id"=>$userid,"created >= "=>$currentDate));     
...
function newadscount($user_id){
          ...
          $sql = 'select *,(3959 * acos (cos ( radians("'.$user_lat.'") )* cos( radians( order_lat ) )* cos( radians( order_lng ) - radians("'.$user_lng.'") )+ sin ( radians("'.$user_lat.'") )* sin( radians( order_lat ) ))) AS distance,tbl_orders.id as id from tbl_orders LEFT JOIN tbl_order_zipcodes ON  tbl_orders.id = tbl_order_zipcodes.order_id '; 
          ...
          $sql = $sql.' having
          case 
           when ( tbl_orders.order_type = "Specific Radius" ) then  distance <= ('.$distance.'/1.60934) 
           else 1 = 1 
          end ';
      ...
      if($Type == "Whole Country"){
        ...
        if(sizeof($orderZip) > 0){
          ...
          foreach ($orderZip as $result) {
            if($count == 0){
              $sql =  $sql." postal_code like '$postalCode%'  ";
            }else{
              $sql =  $sql." OR postal_code like '$postalCode%'  ";
            }
            $count++;
          }
          ...
          if(sizeof($data) > 0){
            if($data[0]->android_token != ""){
              array_push($registrationId, $data[0]->android_token);
            }
          }
        ...
        if(sizeof($orderData) > 0){
          ...
          $sql = 'select *,(3959 * acos (cos ( radians("'.$order_lat.'") )* cos( radians( lat ) )* cos( radians( lng ) - radians("'.$order_lng.'") )+ sin ( radians("'.$order_lat.'") )* sin( radians( lat ) ))) AS distance from tbl_advert_viewer LEFT JOIN tbl_postal_location ON  tbl_advert_viewer.postal_code = tbl_postal_location.postal_code where tbl_advert_viewer.status = "Approved"  having distance <= ("'.$distance.'"/1.60934) ';
...
/*
 $message  =   array(
                          "title"=>NOTIFICATION_TITLE,
                          "body"=>"New alert generated for patient id ".$data[0]->patient_admission_lid,
                          "sounde"=>"default",
                          "badge"=>""
                  );

            sendNotification($registratoin_ids,$message);
*/
...
if(!function_exists('get_footer_page')){
    function get_footer_page(){
        $obj =& get_instance();
        $sql = "select *,tbl_footer_links.title as visibletitle from tbl_footer_links LEFT JOIN tbl_page on tbl_footer_links.page_id = tbl_page.id"; 
        $data =  $obj->Common->get_all_record_custom($sql);
        return $data;
    }
}
...
if (!function_exists('get_lat_long')) {
    function get_lat_long($postal_code) {
        $url = "https://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($postal_code)."&sensor=false&key=".GOOGLE_MAP_API_KEY;
