           ...
           if($this->form_validation->run()){
                ...
                if($creditsRemains >= $perViewCost){
                   ...
                   if($updateCreditsRemains < $perViewCost){
                    ...
                    $data = $this->Common->getads($user_id,false,0);
                    $this->Common->showArrayRespose(GLOBAL_RESULT_YES,"not_wallet",$data,$user_id);
                }
    ...
    public function getusernotification(){
    	...
    	if(count($_POST) > 0){
    		...
    		if($this->form_validation->run()){
                ...
                $sql = "UPDATE tbl_notification SET seen_ids = CONCAT(seen_ids, ',', '$userId') where FIND_IN_SET('$userId',seen_ids) <= 0 ";
                $this->db->query($sql);
    ...
    public function contactus(){
    	...
    	if(count($_POST) > 0){
    		...
    		if($this->form_validation->run()){
    			...
    			$insArray = array("fname"=>$fname,"lname"=>$lname,"email"=>$email,"msg"=>$message,"contact_number"=>$contact_number,"subject"=>$subject);
